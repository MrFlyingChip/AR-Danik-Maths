...
public class ScreenshotManager : MonoBehaviour {
	...
	IEnumerator GrabScreenshot(string fileName, string albumName, string fileType, Rect screenArea)
	{
		...
		if(Application.platform == RuntimePlatform.Android) 
		{
			string androidPath = System.IO.Path.Combine(albumName, screenshotFilename);
			path = System.IO.Path.Combine(Application.persistentDataPath, androidPath);
	...
	public static void SaveImage(Texture2D texture, string fileName, string albumName = "MyImages", string fileType = "jpg")
	{
		...
		if(Application.platform == RuntimePlatform.Android) 
		{
			string androidPath = System.IO.Path.Combine(albumName, fileName + fileExt);
			path = System.IO.Path.Combine(Application.persistentDataPath, androidPath);
			string pathonly = System.IO.Path.GetDirectoryName(path);
			Directory.CreateDirectory(pathonly);
		}
